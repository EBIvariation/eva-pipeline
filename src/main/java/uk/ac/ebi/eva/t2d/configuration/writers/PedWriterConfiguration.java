...
public class PedWriterConfiguration {

    @Bean(T2D_PED_WRITER)
    @StepScope
    public PedWriter pedWriter(T2dTsvParameters parameters) {
        PedWriter writer = new PedWriter();
        writer.setResource(new FileSystemResource(parameters.getPedFileOutput()));
        return writer;
    }
